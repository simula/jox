...
kvm -hda $IMAGE_PATH/$EPC_IMG -netdev tap,id=net0,ifname=tap0,vhost=on -device e1000,netdev=net0,mac=52:54:00:10:34:60 -smp 1 -m 1G -daemonize -vnc :0
sleep 2
...
# start mec servers
kvm -drive file=$IMAGE_PATH/$HTTP_IMG -smp 1 -m 512M -netdev tap,id=net10,ifname=tap4,vhost=on -device e1000,netdev=net10,mac=52:54:00:12:34:bb -netdev tap,id=netMGMT1,ifname=tapM1,vhost=on -device e1000,netdev=netMGMT1,mac=52:54:00:12:aa:aa -daemonize -vnc :1
sleep 2
kvm -drive file=$IMAGE_PATH/$IPERF_IMG -smp 1 -m 512M -netdev tap,id=net11,ifname=tap5,vhost=on -device e1000,netdev=net11,mac=52:54:00:12:34:cc
...
# we add the MEC servers' mgmt interfaces on the same bridge, assuming that the MEC
platform is
