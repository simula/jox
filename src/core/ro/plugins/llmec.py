...
class ll_mec_plugin(object):
    def __init__(self, flex_log_level=None,):
        self.logger = logging.getLogger("jox-plugin.llMEC")
    ...
    def build(self):
        try:
            ...
            self.logger.info(" llMEC default endpoint is {}".format(self.ll_mec_endpoint))
    ...
    def start_notifications(self):
        ...
        while True:
            try:
                if self.ll_mec_plugin_status == self.gv.ENABLED:
                    if self.get_status_llmec_endpoint():
                        ran_stats = self.get_slice_stats()
                        ...
                        message = "Notify >> llMEC endpoint not enabled"
                        self.logger.debug(message)
                    time.sleep(10) # Periodic Notification
                else:
                    message = "Notify >> llMEC plugin not enabled"
                    self.logger.error(message)
                    self.goodbye()
