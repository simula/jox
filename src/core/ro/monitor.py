...
def update_machine_monitor_state(es_host, es_port, machine_id, machine_state, slice_id):
    nssid, container_name, start_time = check_nssid_with_mid(es_host, es_port, machine_id, 'machine_keys', slice_id)
    ...
    update_index_key(es_host, es_port, 'slice_monitor_'+ nssid.lower(), "machine_status", container_name, machine_state,
                     str(launch_time), slice_id)

def update_service_monitor_state(es_host, es_port, service_name, service_state, slice_id):
    nssid, start_time = check_nssid_with_service(es_host, es_port, service_name, slice_id)
    end_time = (datetime.datetime.now()).isoformat()
    
    service_time = (datetime.datetime.strptime(end_time,'%Y-%m-%dT%H:%M:%S.%f'))-(datetime.datetime.strptime(start_time,'%Y-%m-%dT%H:%M:%S.%f'))
    update_index_key(es_host, es_port, 'slice_monitor_'+ nssid.lower(), "service_status", service_name, service_state,
                        str(service_time), slice_id)

def check_nssid_with_mid(es_host, es_port, machine_id, container_type, slice_id):
...
def check_nssid_with_service(es_host, es_port, app_name, slice_id):
    for nsi in range(len(keys_local.keys())):
        nsi_list = list(keys_local)
        if nsi_list[nsi] == slice_id:
            machine_key=keys_local[slice_id]
            ...
            for data in range(len(machine_key)):
                ...
                pass
    service_key = es.get_json_from_es(es_host, es_port, "slice_keys_"+slice_id.lower(), 'machine_keys')
...
def update_index_key(host, port, index_page, container_type, container_name, leaf_key, leaf_value, slice_id):
    try:
                ...
                if machine[num] == container_name:
                    if slice_data[machines][container_name][0][leaf_key] == "0" and leaf_key:
                        ...
                        if leaf_key == 'requirement_wait':
                            slice_data = es.get_json_from_es(host, port, index_page, "relation_status")
                            slice_data[machines][container_name][0][leaf_key] = str(requirement_wait)
                            ES.update(index=index_page, doc_type='post', id=1,
