...
class Monitor(object):
    ...
    def update_machine_monitor_state(self, machine_data, machine_state, current_state_machine, slice_id): # machine_state=add, change
        if self.jesearch.ping():
            ...
            if check_val[0] is not None:
                ...
                self.update_subslice_monitor_index(slice_id.lower()+'_subslice_monitor_' + nssid.lower(),
                                                                 "machine_status",
                                                                 container_name,
                                                                 container_data)
    ...
    def update_service_monitor_state(self, service_data, service_state, current_state_service, slice_id): # machine_state=add, change
        if self.jesearch.ping():
            ...
            if check_val[0] is not None:
                ...
                if state_new == "active":
                    slice_mon_data= self.jesearch.get_json_from_es(slice_id.lower()+'_subslice_monitor_' + nssid.lower(), 'relation_status')
                    if slice_mon_data[0]:
        ...
        if machine_key[0]:
            ...
            for data in range(len(machine_key)):
                ...
                if machine_key[data][service_name][0]['juju_mid'] == str(machine_id):
                    ...
                    slice_mon_data= self.jesearch.get_json_from_es(slice_id.lower()+'_subslice_monitor_' + nssid.lower(), 'machine_status')
                    if slice_mon_data[0]:
                        slice_mon_data=slice_mon_data[1]
                    for num in range(len(slice_mon_data)):
                        application_name = list((slice_mon_data[num]).keys())[0]
        ...
        if service_key[0]:
            ...
            for data in range(len(service_key)):
                ...
                if service_name == str(app_name):
                    ...
                    container_name = service_name
                    slice_mon_data= self.jesearch.get_json_from_es(slice_id.lower()+'_subslice_monitor_' + nssid.lower(), 'service_status')
                    if slice_mon_data[0]:
                        slice_mon_data=slice_mon_data[1]
                    for num in range(len(slice_mon_data)):
                        application_name = list((slice_mon_data[num]).keys())[0]
    ...
    def update_subslice_monitor_index(self, index_page, container_type, container_name, container_data):
        # if container_type == "relation_status":
            # print(container_name, container_data)
        if self.jesearch.ping():
