	...
	def on_request(self, ch, method, props, body):
		...
		if "/resource-discovery" in enquiry["request-uri"]:
			...
			if slcies_context[0]:
				...
				for slice_data in slcies_context:
					...
					for subslcie_name in set_subslices:
						data_subslcie_monitor = ''.join([slice_name,'_subslice_monitor_', str(subslcie_name).lower()])
			...
			if slice_data[0]:
				...
				res[nsi_name] = {}
				for subslcie_name in set_subslices:
					data_subslcie_monitor = ''.join([nsi_name,'_subslice_monitor_', str(subslcie_name).lower()])
					data = self.nfvo_jox.jesearch.get_source_index(data_subslcie_monitor)
